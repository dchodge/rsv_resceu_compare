...
public:
    ...
    NumericMatrix getDebug(VectorXd currentParamValues);
    ...
    void ParameterValuesforODE(VectorXd currentParamValues){
        ...
        for (int a = 0; a < 12; a++)
            this->ep_t.push_back(exp(parameterValues["ep1"] +
        for (int a = 12; a < 16; a++)
            this->ep_t.push_back(exp(parameterValues["ep2"] +
        for (int a = 16; a < 23; a++)
    ...
    double evaluateTimeStepLogLikelihood(int metricNo){
        ...
        for (int a = 0; a < this->A; a++){
            ...
            if (dataNewInfections > this->modelIncidencePerTime[a]){
             ...
             /*   Rcpp::Rcout << "dataNewInfections: " << dataNewInfections << std::endl;
                Rcpp::Rcout << "this->modelIncidencePerTime[a]: " << this->modelIncidencePerTime[a] << std::endl;
                Rcpp::Rcout << "metricNo: " << metricNo << std::endl;
        ...
        if (std::isinf(ll) || std::isnan(ll)){
         // Rcpp::Rcout << "LL infinite here" << std::endl;
          this->loglikelihoodError=true;
        }
        return ll;
    }
 ...
 //   ga1 = 1.0/(finELL->parameterValues["ga0"]*finELL->parameterValues["g1"]);
  ...
  void operator() (  vector< double >  &x , vector< double >  &dxdt , const double  t )
  {
    ...
    for (int a = 0; a < A ; a++)
    {
        ...
        for (int b = 0; b < A ; b++)
        {
            I_temp += (x[3 + 23*b]*alpha_i+x[4+23*b]+a1*(x[8+23*b]*alpha_i+x[9+23*b])+a2*(x[13+ 23*b]*alpha_i+x[14+23*b])+a3*(x[18+23*b]*alpha_i+x[19+23*b]))*(qp*(contactMatrixPhy(a, b)+qc*contactMatrixCon(a, b)))/((double)populationPerAgeGroup[b]);
        }
...
NumericMatrix EvaluateLogLikelihood::getDebug(VectorXd currentParamValues)
{
  // Restart values
  ...
  NumericMatrix sampleDailyIncidence(365, 575);
