...
# 100000 / 12-age groups / 12 calendar months
100000 / 12 / 12

observational_data_list <- vector(mode = "list", length = 25)
for (i in 1:25) {
    if ( i < 25)
        observational_data_list[[i]] <- apply(observationalData_raw[age_strat[[i]][1]:age_strat[[i]][2]], 1, mean) / 1000 * 100000 * (ageGroupBoundary[i + 1] - ageGroupBoundary[i]) / 12
    else 
         observational_data_list[[i]] <- apply(observationalData_raw[age_strat[[i]][1]:age_strat[[i]][2]], 1, mean) / 1000 * (population - 100000*ageGroupBoundary[i-1]) / 12
}
...
save(resceudata, file = here::here("data", "inputs", "rsv_data_resceu.RData"))
